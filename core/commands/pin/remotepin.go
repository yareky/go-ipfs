...
`,
	...
	Options: []cmds.Option{
		...
		cmds.StringsOption(pinCIDsOptionName, "Return pins for the specified CIDs (comma-separated)."),
		cmds.StringsOption(pinStatusOptionName, "Return pins with the specified statuses (queued,pinning,pinned,failed).").WithDefault([]string{"pinned"}),
	},
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		ctx, cancel := context.WithCancel(req.Context)
		defer cancel()
...
`,
	...
	Options: []cmds.Option{
		...
		cmds.StringsOption(pinCIDsOptionName, "Remove pins for the specified CIDs."),
		cmds.StringsOption(pinStatusOptionName, "Remove pins with the specified statuses (queued,pinning,pinned,failed).").WithDefault([]string{"pinned"}),
